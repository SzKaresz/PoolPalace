            ...
            if (hasError) {
                if (firstErrorField) {
                     ...
                     if (firstTooltipInstance) {
                         ...
                         const reShowTooltip = () => {
                             if (firstErrorField.dataset.persistTooltip === 'true' && firstErrorField.classList.contains('is-invalid')) {
                                  setTimeout(() => {
                                     if (firstErrorField.classList.contains('is-invalid')) {
                                         firstTooltipInstance.show();
                                     }
                                  }, 50);
                             }
                         };
                         firstErrorField.removeEventListener('hidden.bs.tooltip', reShowTooltip);
                         firstErrorField.removeEventListener('blur', reShowTooltip);
                         firstErrorField.addEventListener('hidden.bs.tooltip', reShowTooltip, { once: true });
                         firstErrorField.addEventListener('blur', reShowTooltip);
                     }
                }
                return;
            }
